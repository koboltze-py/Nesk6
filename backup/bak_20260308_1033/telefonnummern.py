...
class TelefonnummernWidget(QWidget):
    """Telefonnummern-Verzeichnis aus Excel-Import."""
    ...
    def _auto_import_if_empty(self):
        """Importiert Excel beim ersten Start, wenn die DB noch leer ist."""
        try:
            if ist_db_leer():
                n = importiere_aus_excel(clear_first=True)
                self._status_lbl.setText(f"✅  Erstimport: {n} Einträge geladen.")
